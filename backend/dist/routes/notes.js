...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const notesSnapshot = await firebase_1.db.ref('notes').orderByChild('userId').equalTo(userId).once('value');
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const newNoteRef = firebase_1.db.ref('notes').push();
        const newNote = {
            userId,
            content,
...
router.delete('/:id', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const noteRef = firebase_1.db.ref(`notes/${noteId}`);
